...
export const makeBootstrapPowers = async (
  ...
  const boardAux = async obj => {
    return chainStorage.getBody(
      `mockChainStorageRoot.boardAux.${await E(board).getId(obj)}`,
      roMarshaller,
    );
  };
  ...
  console.log('at the end of makeBootstrapPowers #####', {
    powers,
    vatAdminState,
    vatPowers: vatAdminState.getVatPowers(),
  });
...
const getNameHubAdmin = 'namesByAddressAdmin';

const getProduce = getProp('produce');
export const bootAndInstallBundles = async (t, bundleRoots) => {
  t.log('bootstrap');
  const powersKit = await makeBootstrapPowers(t.log);
  console.group('---------- inside bootANdIntallBudles----------');
  console.log('------------------------');
  console.log('powersKit::', powersKit);
  console.log('------------------------');
  console.log('::');
  console.log('------------------------');
  console.groupEnd();
